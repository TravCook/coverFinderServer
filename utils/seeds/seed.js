    ...
    for (let sport of sports) {
        ...
        if (sport.name === 'baseball_mlb') {
            ...
            if (pastGames.length > 10) {
                ...
                await predictions(upcomingGames, [], model, sport, false, false, pastGames.sort((gameA, gameB) => new Date(gameB.commence_time) - new Date(gameA.commence_time)).slice(0, sport['hyperParams.historyLength']))
    ...
    for (const sport of sports) {
        ...
        if (sportGamesSQL.length > 0) {
            ...
            if (fs.existsSync(modelPath)) {
                ...
                await predictions(sportGamesSQL, [], model, sport,false, false, allPastGamesSQL.sort((gameA, gameB) => new Date(gameB.commence_time) - new Date(gameA.commence_time)).slice(0, sport['hyperParams.historyLength']))
            } else {
