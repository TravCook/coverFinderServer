...
class Outcomes(Base):
    ...
    marketId: Mapped[int] = mapped_column(ForeignKey("Markets.id"))
    teamId: Mapped[int] = mapped_column(ForeignKey("Teams.id"), nullable=True)
    ...
    #relationships
    market: Mapped["Markets"] = relationship("Markets", foreign_keys=[marketId], back_populates="outcomes")
    teams: Mapped["Teams"] = relationship("Teams", foreign_keys=[teamId])
